...
{
    private static final int DIALOG_TAP_SLOP = 24;
    ...
    private final boolean[] emptyTapCandidate = new boolean[10];
    private final int[] emptyTapStartX = new int[10];
    private final int[] emptyTapStartY = new int[10];
...
/*        if (Q3EGL.usegles20)
    ...
    boolean OnTouchEvent(MotionEvent event)
    {
        ...
        if ((actionMasked == MotionEvent.ACTION_DOWN) || (actionMasked == MotionEvent.ACTION_POINTER_DOWN))
        {
            ...
            emptyTapCandidate[pid] = true;
            emptyTapStartX[pid] = x;
            emptyTapStartY[pid] = y;
            for (TouchListener tl : touch_elements)
            {
                if (tl.isInside(x, y))
                {
                    fingers[pid].target = tl;
                    emptyTapCandidate[pid] = false;
                    break;
                }
            }
        }
        ...
        if (actionMasked == MotionEvent.ACTION_MOVE)
        {
            int x = (int) event.getX(actionIndex);
            int y = (int) event.getY(actionIndex);
            if (Math.abs(x - emptyTapStartX[pid]) > DIALOG_TAP_SLOP || Math.abs(y - emptyTapStartY[pid]) > DIALOG_TAP_SLOP)
            {
                emptyTapCandidate[pid] = false;
            }
        }
        ...
        {
            if (actionMasked != MotionEvent.ACTION_CANCEL && fingers[pid].target == null && emptyTapCandidate[pid] && Q3E.callbackObj != null && !Q3E.callbackObj.inLoading)
            {
                Q3E.sendKeyEvent(true, Q3EKeyCodes.KeyCodes.K_ENTER, 0);
                Q3E.sendKeyEvent(false, Q3EKeyCodes.KeyCodes.K_ENTER, 0);
            }
